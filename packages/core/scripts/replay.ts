...
for (const t of replayHours(fx)) {
  ...
  console.log(
    `${stamp} UTC | hotspots ${inc.front.length} (pass ${inc.data_time?.slice(11, 16) ?? "-"}) | wind ${inc.wind_text.split(",")[0]} | spread ${inc.spread_kmh?.toFixed(1) ?? "-"} km/h | at risk ${inc.places.length}`,
  );
  for (const p of inc.places.slice(0, perHour))
    console.log(
      `    ${p.kind.padEnd(13)} ${p.name.slice(0, 45).padEnd(45)} ${String(p.distance_km).padStart(5)} km, fire to the ${p.fire_direction}, ${p.arrival_estimate} (${p.reason})`,
    );
}
