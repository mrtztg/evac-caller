...
| 2026-09-20 12:45 | M3: how does the dashboard see the live phone calls? | Shared event file / small HTTP server in the bot / no live calls on the dashboard | Shared event file `data/call-events.jsonl` | The bot appends one JSON line per call event; the web page reads the file on the server. No new port to fail on stage, and it survives a restart of either side. The same idea sets the replayed hour from the dashboard slider. |
